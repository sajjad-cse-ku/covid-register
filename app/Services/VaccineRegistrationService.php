...
class VaccineRegistrationService
{
    ...
    {
        try {
            $vaccineLimit = VaccineCenter::where('id', $centerId)->value('daily_limit');
            $latestDate = VaccineRegister::where('center_id', $centerId)->max('scheduled_date');
        } catch (\Exception $e) {
        ...
        $registrationCount = VaccineRegister::where('scheduled_date', $latestDate)->count();
    ...
    public function data(object $reg): array
    {
        return [
            'name' => $reg->name,
            'email' => $reg->email,
